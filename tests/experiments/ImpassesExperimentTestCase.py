...
class ImpassesExperimentTestCase(ExperimentTestCase):
    ...
    def test_example_throughput(self):
        ...
        # 2b TEST: GOAL-THAT-WILL-BE-BLOCKED is active
        ...
        self.assertDecisionExists(agent, status=Decision.Status.BLOCKED, goal="@SELF.GOAL.1", plan="@SELF.PLAN.1", step="@SELF.STEP.1", impasses=["@EXE.SOME-SUBGOAL.1"])
        self.assertDecisionExists(agent, status=Decision.Status.EXECUTING, goal="@EXE.SOME-SUBGOAL.1", plan="@EXE.PLAN.3", step="@EXE.STEP.3", callbacks=["@SELF.CALLBACK.1"])
        ...
        self.assertDecisionExists(agent, status=Decision.Status.FINISHED, goal="@EXE.SOME-SUBGOAL.1", plan="@EXE.PLAN.3", step="@EXE.STEP.3", callbacks=[])
        ...
        self.assertGoalExists(agent, isa="@EXE.GOAL-THAT-WILL-BE-BLOCKED", status=Goal.Status.ACTIVE)
        self.assertGoalExists(agent, isa="@EXE.SOME-SUBGOAL", status=Goal.Status.SATISFIED)
        self.assertDecisionExists(agent, status=Decision.Status.EXECUTING, goal="@SELF.GOAL.1", plan="@SELF.PLAN.1", step="@SELF.STEP.1", callbacks=["@SELF.CALLBACK.1"])
