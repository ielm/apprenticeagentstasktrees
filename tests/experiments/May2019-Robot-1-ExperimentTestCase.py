...
class Jan2019Experiment(ExperimentTestCase):
    ...
    def test_robot_1(self):
        ...
        self.assertEqual(Frame("@ENV.HUMAN.1"), Frame("@" + XMR(Frame("@OUTPUTS.XMR.5")).space().name + ".GREET.1")["THEME"].singleton())
        ...
        self.assertGoalExists(agent, isa="@EXE.ACKNOWLEDGE-LANGUAGE-INPUT", status=Goal.Status.SATISFIED, query=lambda goal: goal.resolve("$tmr")["REFERS-TO-SPACE"].singleton() == "TMR#2")
        self.assertGoalExists(agent, isa="@EXE.RESPOND-TO-QUERY", status=Goal.Status.PENDING, query=lambda goal: goal.resolve("$tmr")["REFERS-TO-SPACE"].singleton() == "TMR#2")
        ...
        self.assertGoalExists(agent, isa="@EXE.RESPOND-TO-QUERY", status=Goal.Status.ACTIVE, query=lambda goal: goal.resolve("$tmr")["REFERS-TO-SPACE"].singleton() == "TMR#2")
        self.assertFalse(Effector(Frame("@SELF.VERBAL-EFFECTOR.1")).is_free())
        self.assertEqual(Frame("@EXE.SPEAK-CAPABILITY"), Effector(Frame("@SELF.VERBAL-EFFECTOR.1")).on_capability())
        self.assertEqual(Frame("@OUTPUTS.XMR.7"), Effector(Frame("@SELF.VERBAL-EFFECTOR.1")).on_output())
        self.assertEqual(Frame("@XMR#3.FETCH.1"), Frame("@" + XMR(Frame("@OUTPUTS.XMR.7")).space().name + ".DESCRIBE.1")["THEME"].singleton())
        ...
        self.assertGoalExists(agent, isa="@EXE.RESPOND-TO-QUERY", status=Goal.Status.SATISFIED, query=lambda goal: goal.resolve("$tmr")["REFERS-TO-SPACE"].singleton() == "TMR#2")
        self.assertTrue(Effector(Frame("@SELF.VERBAL-EFFECTOR.1")).is_free())
        ...
        self.assertFalse(Effector(Frame("@SELF.PHYSICAL-EFFECTOR.1")).is_free())
        self.assertEqual(Frame("@EXE.FETCH-OBJECT-CAPABILITY"), Effector(Frame("@SELF.PHYSICAL-EFFECTOR.1")).on_capability())
        self.assertEqual(Frame("@OUTPUTS.XMR.8"), Effector(Frame("@SELF.PHYSICAL-EFFECTOR.1")).on_output())
        self.assertEqual(Frame("@ENV.BRACKET.2"), Frame("@" + XMR(Frame("@OUTPUTS.XMR.8")).space().name + ".FETCH.1")["THEME"].singleton())
    ...
    def test_1_2(self):
        ...
        # Pb) There is an instance of "Jake", who is known, present, and taught the robot the chair instructions
        agent.load_knowledge("backend.resources.experiments", "Jan2019_1_2.environment")
        agent.load_knowledge("backend.resources.experiments", "Jan2019_1_2.knowledge")
