...
def merge_children(a, b, alist, blist):
    ...
    while len(a_not_mapped) + len(b_not_mapped) > 0:
        ...
        for child in (b_not_mapped if maxnode is amax else a_not_mapped):
            ...
            S.append(min([S[-1]] + [(s[0] - set(otherleaves[child]), s[1] + [child]) for s in S if
                                    s[0] >= set(otherleaves[child])], key=lambda x: len(x[0])))
            # pair of (smallest possible subset of X's leaves after subtracting some things in the range b1...bi, set of those things)
            if len(S[-1][0]) == 0:
                break
        else:
            raise RuntimeError(
                "Cannot merge trees: no child set found corresponding to node " + maxnode.name + "; unmapped nodes remaining: %d\n" % len(
                    S[-1][0]) + str(S) + "; candidates are: " + str(
                    [node.name for node in (b_not_mapped if maxnode is amax else a_not_mapped)]))
        ...
        if maxnode is bmax:
            ...
            for i in range(len(a.children)):
                ...
                elif a.children[i] in mappingdict and (
                        (i > pos) != (b.children.index(mappingdict[a.children[i]]) > b.children.index(maxnode))):
