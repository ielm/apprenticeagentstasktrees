...
def add_concepts_from_tree(node, timestamp=None):
    ...
    if record is None:
        ...
        if "INSTRUMENT" in event:
            props.append(
                {"filler": node.tmr[event["INSTRUMENT"]]["concept"].lower(), "slot": "instrument", "facet": "sem"})
            ...
            if property["slot"] == "has-event-as-part" and type(property["filler"]) is list:
                ...
                while match:
                    ...
                    if oldparts[i] == newparts[j]:
                        ...
                        change = True
                        for sequence in onto.aggregate(
                                [{"$match": {"name": oldparts[i]}}, {"$unwind": "$localProperties"},
                                 {"$match": {"localProperties.slot": "has-event-as-part"}},
                                 {"$project": {"_id": 0, "parts": "$localProperties.filler"}}]):
                        ...
                        if not match:
                            for sequence in onto.aggregate(
                                    [{"$match": {"name": newparts[j]}}, {"$unwind": "$localProperties"},
                                     {"$match": {"localProperties.slot": "has-event-as-part"}},
                                     {"$project": {"_id": 0, "parts": "$localProperties.filler"}}]):
        ...
        if change:
            onto.update_one({"name": name}, {
                "$push": {"localProperties": {"facet": "sem", "slot": "has-event-as-part", "filler": newparts}},
